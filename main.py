...
class win(tk.Tk):
    def __init__(self):
        ...
        menu.add_command(label="Guardar datos", command=lambda: self.open_csv())
        menu.add_command(label="Cargar datos guardados", command=lambda: self.save_sheet())
    ...
    def open_csv(self):
        ...
        if not filepath or not filepath.lower().endswith((".csv", ".tsv")):
            return
        try:
            with open(Path(filepath), "r") as filehandle:
